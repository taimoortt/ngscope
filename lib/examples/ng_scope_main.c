...
void init_plots(char* suffix_logfile);
...
void parse_args(prog_args_t* args, int argc, char** argv)
{
  ...
  while ((opt = getopt(argc, argv, "adAogliIpPcOCtdDFRqnvrfuUsSZyWMNBTGQL")) != -1) {
    switch (opt) {
      case 'i':
      ...
      case 'Q':
        ...
        break;
      case 'L':
        args->suffix_logfile = argv[optind];
        break;
      default:
        usage(args, argv[0]);
        exit(-1);
...
int main(int argc, char** argv)
{
  ...
  if (!prog_args.disable_plots) {
    init_plots(prog_args.suffix_logfile);
    sleep(1);
  }
#endif /* ENABLE_GUI */
...
void* plot_thread_run(void* arg)
{
  ...
  char fileName[100];
  sprintf(fileName,"/home/netlab/Desktop/%s.log", suffix);
  ...
  if (!prog_args.disable_plots_except_constellation) {
    plot_real_init(&pce);
    plot_real_setTitle(&pce, "Channel Response - Magnitude(mark1)");
  ...
  while (1) {
    sem_wait(&plot_sem);
    fprintf(fd, "tti: %d snr_db: %f rsrp_db: %f noise_db: %f\n",
      dl_sf.tti, ue_dl.chest_res.snr_db,
      ue_dl.chest_res.rsrp_dbm,
      ue_dl.chest_res.noise_estimate_dbm);
...
void init_plots(char* suffix_logfile)
{
  ...
  if (pthread_create(&plot_thread, NULL, plot_thread_run, (void*)suffix_logfile)) {
